...
### 2. 表示範囲スクリーンショット
- 現在のブラウザ画面に表示されている範囲のスクリーンショットを取得
- 高速でシンプルなキャプチャ機能
- 取得した画像は自動的にダウンロード
...
#### 表示範囲スクリーンショット
Chrome の標準 API を使用した高速キャプチャ：

**技術仕様:**
- **Chrome Tabs API**: `chrome.tabs.captureVisibleTab` を使用
- **高速処理**: 現在表示中の画面を瞬時にキャプチャ
- **軽量実装**: デバッガー不要でシンプルな処理

**主な特徴:**
- **即座にキャプチャ**: 表示範囲のみを高速で取得
- **軽量処理**: システムリソースを最小限に抑制
- **標準品質**: ブラウザネイティブの画質でキャプチャ
...
5. **表示範囲スクリーンショット**: Chrome Tabs API 使用
   - 高速キャプチャ: 現在表示中の範囲を瞬時に取得
...
3. フローティングメニューから機能を選択：
   ...
   - 🖼️ **表示範囲スクリーンショット**: 現在表示中の範囲を高速キャプチャ
